...
const Portfolio = () => {
  ...
  return <section id="portfolio" className="py-24 bg-[#FFDEE2] relative">
      {/* Background elements */}
      ...
      <div className="container-section relative z-10">
        ...
        <motion.div className="grid grid-cols-1 gap-16 mt-12" variants={container} initial="hidden" whileInView="show" viewport={{
        once: true
      }}>
          {projects.map(project => <motion.div key={project.id} className="bg-white rounded-xl overflow-hidden shadow-xl border border-gray-100" variants={item}>
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="h-64 md:h-auto relative group overflow-hidden bg-white flex items-center justify-center p-8">
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-start p-6 z-10">
                    <a href={project.link} className="inline-flex items-center gap-2 bg-white text-portfolio-navy px-4 py-2 rounded-md text-sm font-medium hover:bg-portfolio-navy hover:text-white transition-colors duration-300" target="_blank" rel="noopener noreferrer">
                      View Project <ExternalLink className="h-4 w-4" />
                    </a>
                  </div>
                  <img src={project.image} alt={project.title} className="max-w-[80%] max-h-[80%] object-contain transform group-hover:scale-110 transition-transform duration-700" />
                </div>
                <div className="p-8 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      {project.tags.map(tag => <span key={tag} className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded-full">{tag}</span>)}
                    </div>
                  ...
                  <div className="mt-8">
                    <a href={project.link} className="inline-flex items-center text-portfolio-navy group/link border-b border-portfolio-navy/30 pb-1 hover:border-portfolio-navy transition-colors" target="_blank" rel="noopener noreferrer">
                      View Project 
        ...
        duration: 0.6
          ...
          <div className="bg-gradient-to-r from-portfolio-light-gray via-white to-portfolio-light-gray py-8 px-4 rounded-xl">
            ...
            <div className="flex justify-center space-x-3">
              <motion.span className="h-3 w-3 rounded-full bg-portfolio-orange" animate={{
              y: [0, -10, 0],
              scale: [1, 1.1, 1]
            }} transition={{
              repeat: Infinity,
              ...
              <motion.span className="h-3 w-3 rounded-full bg-portfolio-teal" animate={{
              y: [0, -10, 0],
              scale: [1, 1.1, 1]
            }} transition={{
              repeat: Infinity,
              ...
              <motion.span className="h-3 w-3 rounded-full bg-portfolio-navy" animate={{
              y: [0, -10, 0],
              scale: [1, 1.1, 1]
            }} transition={{
              repeat: Infinity,
